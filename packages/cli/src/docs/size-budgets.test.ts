...
const claudeAdapterFiles = factoryClaudeAdaptersPresent
  ? readdirSync(claudeAdapterDir)
      .filter((f) => f.endsWith(".md"))
      .map((f) => `.claude/commands/${f}`)
  : [];

describe("size-budgets: .claude/commands/*.md adapters (target ceiling: 25 lines / 2048 B / 500 chars)", () => {
  const MAX_LINES = 25;
  const MAX_BYTES = 2048;
  const MAX_LONGEST_LINE = 500;

  it.skipIf(!factoryClaudeAdaptersPresent)("has the expected non-empty membership", () => {
    expect(claudeAdapterFiles.length).toBeGreaterThan(15);
  });

  for (const target of claudeAdapterFiles) {
    it(`${target} stays within budget`, () => {
      const m = measureFile(target);
      expect(m.lines, `${target} lines`).toBeLessThanOrEqual(MAX_LINES);
      expect(m.bytes, `${target} bytes`).toBeLessThanOrEqual(MAX_BYTES);
