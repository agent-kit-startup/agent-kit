...
describe("generateClaudeKitLoadArtifacts", () => {
  ...
  it("keeps factory dogfood files identical to generator snippets", async () => {
    expect(await readFile(path.join(REPO_ROOT, CLAUDE_MD_REL), "utf8")).toBe(renderClaudeMd());
    expect(await readFile(path.join(REPO_ROOT, AGENT_KIT_COMMAND_REL), "utf8")).toBe(
      renderAgentKitCommand(),
    );
  });
