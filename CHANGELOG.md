...
The format follows [Keep a Changelog](https://keepachangelog.com/en/1.0.0/), and this project adheres to [Semantic Versioning](https://semver.org/). PATCH is for bug fixes and post-tag republish-class fixes. A hold (same SemVer, no new tag) is allowed when the existing npm tarball does not need to change. Shipping only `x.y.0` is not a second cadence.

**Public vs private:** the private factory tree may keep a full developer log. Public surfaces (public GitHub `CHANGELOG.md`, GitHub Release notes, landing product notes) publish only consumer and contributor product notes. Fence factory-only bullets (CI/CD internals, landing staging/promote, Hostinger/deploy plumbing, internal ADR chatter, dogfood-only mechanics) in `<!-- changelog-private -->` ... `<!-- /changelog-private -->` blocks, or under a `### Internal` heading. Public-sync strips those fences. Preview: `node scripts/public-changelog.mjs`. Landing stamp: `node scripts/public-changelog.mjs --version X.Y.Z --blurb` then `pnpm landing:update-release -- --version X.Y.Z --notes "<blurb>"`. Never pass `CHANGELOG.md` as `--notes-file`.
...
- **Public CI adapter size pins no longer list `.claude/commands` while the test file loads.** `describe.skipIf` still runs its callback, so v5.10.1 still threw `ENOENT` on public CI. The listing now runs only when that directory exists.

## [5.10.1] - 2026-09-22

### Fixed

- **Public CI size pins skip factory-only Claude adapters.** The `.claude/commands` size-budget suite no longer reads that directory when it is absent. Public-sync does not ship those adapters. Private checkouts still pin them.

## [5.10.0] - 2026-09-21

### Added

- **Mission Control live view on the run lane.** `agent-kit run`, `run-plan` and `run-plan-all` in a terminal now show the run as Mission Control instead of status lines: a Mission panel with progress bars (to-dos of the current plan; plans of the `run-plan-all` queue, read from HANDOFF without touching the cursor), a scrolling Flight Log of the same status lines, a Checklist of the queue or the plan's to-dos, and a Crew Monitor with the backend, pid, elapsed time, cost, turns and the tool in flight. A HITL gate is rendered inside the Mission panel with an input line: number or label replies, anything else is sent as `other`, an empty line, `skip`, `cancel` or Ctrl-C stops; `q` outside a gate stops the run with a `stopped-by-operator` record. The terminal is restored on every exit and the reply stamps and run summary print beneath the last frame. New `--plain` keeps the status lines; non-TTY, `NO_COLOR` and `CI` are always plain. No new dependency; `.cursor/loop-logs/` keeps the complete NDJSON.
- **Headless HITL relay: answer the numbered list from the terminal.** `agent-kit run`, `run-plan` and `run-plan-all` on the `claude` backend keep one `stream-json` session per run with stdin open: the prompt goes in as the first `user` event, a gate pauses the run, the terminal shows the ask-id and the options, and the operator's number or label is delivered as one `HITL_REPLY: <ask-id> | operator reply <n> | <label>` event that the child echoes into `.cursor/loop-logs/` (provenance) and the exit summary repeats. Empty line, EOF, `skip`, `cancel` or Ctrl-C write nothing and stop the run with a `stopped-by-operator` record; a non-TTY stdin stops the same way, never a default answer. New opt-in `--no-hitl` (CI) ends the run at a gate with exit code 4. Adapters and the `hitl-gates` skill now print a `HITL_GATE: <ask-id> | <labels>` line before every numbered list, with an id per gate; pre-sentinel lists are still detected from their wording and marked as a fallback. `cursor-agent` has no input channel: a gate there stops the run and says so.
- **`/qa` L0 slash.** Operators can test a release, reproduce a bug, or verify CHANGELOG feat/fix/chore claims against the install journey, tests, and hygiene. Playbook: `.cursor/skills/core/qa/`. No new L0 agent (`test-suites` stays pack `quality`). Reports pass/fail; residuals go through `/backlog-add`. ADR `2026-09-16_qa-l0-slash-no-new-agent.md`. Never `/git-prod`.
- **Cursor awareness feature-keyword digest.** `agent-kit cursor-awareness --check` compares changelog headings and slash commands to `docs/cursor-native-audit.md` and `docs/cursor-3-features.md`. A named surface missing from both files is an advisory gap. Check still never rewrites the inventory (`applyRecommended` stays false).
- **HITL gates core skill** (`.cursor/skills/core/hitl-gates/`). Kit-wide Ask questions contract, exact labels, and numbered-list fallback as path 1. Consumer L0 overlay ships `SKILL.md` plus `procedures.md` (tick, intake, queue SoT). ADR `2026-07-29_consumer-l0-overlay-agents-optional.md` unchanged.
- **Hosted harness paper (Proposed).** Factory extract, ACP client architecture, community incentive extension, basic vs advanced line, license-options research, and an implementation gate that is not satisfied. ADR `2026-09-17_hosted-harness-stateless-control-plane.md` stays Proposed and does not auto-supersede `2026-09-03`. Local ACP two-agent handshake recorded as FAIL (adapters not on PATH). No hosted service code.
- **Scanner recognizes Dart/Flutter and package-per-subdirectory monorepos.** `pubspec.yaml` joins the project markers, with a dart/flutter language branch (`flutter test` / `dart test`, `dart analyze --fatal-infos`, `dart format --output=none --set-exit-if-changed .`). When the repository root has no manifest at all, the scanner now scans one level of subdirectories for any known marker (skipping `node_modules`, `.git`, `build`, `.dart_tool`) and reports `workspaces: true` with each member path as evidence, so `purpose` stops defaulting to `documentation` for a repo whose packages live under `core/` + `app/`-style subdirectories. `.cursor/agent-kit.config.json#stack` also gains an operator-confirmed override, parity with `purpose` and `git.provider`: once set with a language and at least one command, it is the source of truth on every later scan. `quality.validation` readiness now also accepts a repeatable command parsed from GitHub Actions `run:` steps (regex-only, no new dependency) as evidence when the scanner's own stack detection found none.

### Changed

- **Cross-repo memory inventory.** Research note `docs/research/cross-repo-memory-inventory-2026-09.md` inventories existing ADRs, processed dogfood, and adjacent boards. Public five-layer matrix and capability inventory now name the per-checkout memory bound and reject a fleet HANDOFF board (ADR 2026-09-15). Kitchan and hosted-harness stay adjacent-not-owner. No AgentStore product, hosted team-context store, or L1 memory-consolidation unlock. Leftover remote after merged PR #886 deleted.
- **Mission Kit landing hero copy refresh (zip 7 v2).** Centered H1 "Plan, build, review, and ship anything to the real world.", expanded lead (orchestration / memory / context / commands / rules / hooks / skills), Install Mission Kit and Watch Demo, video below CTAs. Public release pill remains Mission Kit 5.9.0.
- **SessionStart HANDOFF excerpt has a byte cap.** The context the `agent-kit hook session-start` adapter injects (Cursor `sessionStart` and Claude Code `SessionStart`, both formats) keeps the first 60 lines of `.cursor/HANDOFF.md` but now within 12 KB: the machine fields Mission Control reads (`Plan`, `Last updated`, `Mode`, `In progress`, `Next*`, `Backlog plans`, `Parked plans`, `Run queue`, `Queue cursor`, `Queue status`, with their indented sub-bullets) are always whole, prose bullets share the rest and end in a visible ` …[truncated N chars]` marker, and a closing line says how many were truncated. A HANDOFF that already fits is injected unchanged. On a 60-line HANDOFF with multi-thousand-character bullets the excerpt drops from about 61 KB (factory) or 38 KB (a consumer run) to under 12 KB, so every headless turn re-reads a smaller cached prefix.
- **Headless runs print status lines, not raw NDJSON.** `agent-kit run`, `run-plan` and `run-plan-all` on both backends (the `cursor-agent` tool events follow its published `tool_call` shape) now render the agent stream in the terminal as readable lines: assistant text, tool calls with a one-line argument summary, tool results (ok/error), hooks collapsed to one line, rate-limit warnings and the final result with status, cost and duration; thinking, init and catalog events stay silent. Non-TTY, `NO_COLOR` and `CI` print the same lines without color. The `.cursor/loop-logs/` file keeps the complete NDJSON, redaction unchanged.
- **Headless HITL transport decided (ADR `2026-09-19_headless-hitl-transport-and-live-tui.md`, Accepted).** `agent-kit run` / `run-plan` / `run-plan-all` on the `claude` backend will keep one `stream-json` child per run with stdin open and relay operator answers as `user` events; the numbered-list fallback gains a `HITL_GATE:` sentinel line and replies are stamped `HITL_REPLY:`. Point 7 of ADR `2026-08-27` is amended so the live Mission Control loop also covers the headless run lane (dependency lock unchanged). Decision only; the renderer, relay and TUI live mode ship in later phases.
- **Cursor native-audit keep-up (2026-09-16).** `docs/cursor-native-audit.md` and `docs/cursor-3-features.md` restamped for classified post-Projects surfaces (Custom modes, `/goal`, `/loop`, Origin, self-hosted machines, and related ignore rows). Projects ADR `2026-09-12` and Plan Mode ADR `2026-09-15` unchanged. Research: `docs/research/cursor-changelog-keep-up-2026-09.md`.
- **Kit plans stay SoT over host Plan Mode.** If Cursor already has a native plan, copy it into `.cursor/plans/` then run Gate A. Host Plan Mode does not replace Gate A/B. ADR `2026-09-15_kit-plans-sot-over-host-plan-mode.md`. Native-audit inventory and `.cursor/project-context.md` restamped to live **5.9.0** manifests (dated snapshot).
- **`/run-plan` L0 procedure is under 80 lines** (was 381). `/run-plan-all` and `/start-project` keep Goal, five hard stops, Ask labels, and a link to the skill. `hitl-ask-questions.mdc` stays the short always-applied rule.
- **Default-path HITL is one Ask at command entry.** Inbox, owed-close, and landing Asks are opt-in or post-default. Numbered-list fallback is path 1 (mandatory when Ask questions is missing). Ask questions matrix records a 2026-09-15 Grok 4.6 main-Agent re-probe (present); Auto and Sonnet were not re-probed this pass.
- **Session hard-rules Gate A** now matches `/start-project`: no-active-plan options include `Write plan and add to backlog` (`write / write+backlog / modify / cancel`).
- **Consumer L0 drops `/kit-staging` and `/kit-prod`.** Factory keeps the files. Native `/git-staging` / `/git-prod` stay git-only with one pointer bullet. `/start-project` critical path is Broad Intake + Gate A (product-context extract only when the payload mixes personal and product; Major Tom is not intake). Mission Control TUI stays a third-surface viewer (ADR `2026-08-27_cli-native-mission-control-tui-thin-deps-third-surface.md`); no new CLI deps.
- **Agent Persona chrome runs after the command Ask.** Default chat chrome has no cockpit emoji. Personas stay chat tone only (`2026-07-26_agent-persona-vs-interface-skin.md` unchanged).
- **`/git-staging` stays on the routine after `gh` HTTP 401.** After a successful SSH push, auth failure prints the compare URL and waits. It does not invent dotenv, L0, CHANGELOG, or overlay-hash product. Troubleshooting no longer treats `gh auth login` as the first move in that case. ADR `2026-09-16_git-staging-stay-on-routine-no-invented-product.md`.
- **Consumer docs cover the headless TUI, `--plain`, `--no-hitl` and the HITL relay transport.** `docs/consumer-configuration.md`'s CLI table and `docs/getting-started.md` now document the Mission Control live view on `run` / `run-plan` / `run-plan-all`, the `--plain` and `--no-hitl` flags, and the sentinel-based relay (`HITL_GATE:` / `HITL_REPLY:`) that answers a gate from the terminal and resumes the same session, with the same-model stop conditions and the `git-prod` / `kit-prod` refusal restated. The `hitl-gates` skill gained a matching "interactive terminal" paragraph distinguishing this from the unattended-CI, one-tick-per-agent runner it already documented. `.cursor/commands/dashboard.md` step 4 now names the browser MCP per IDE lane and gets a Troubleshooting row for an extension error page while the server answers 200 (site-permission gap, not a dead server; do not retry navigate).
- **`/agent-kit-onboard` essential-deferral rule narrowed to match the code, and the code narrowed to match the intent.** An essential readiness check can be completed by an explicit deferral only when the scanner has zero recovery actions for it (e.g. `stack.detected` on an unrecognized language); an essential check with a listed action always resolves through that action. `agent-kit doctor` now prints `Deferred essential: <check-id> — <reason>` (and `deferredEssentials` in `--json`) whenever this path is used. ADR `decisions/2026-07-28_onboarding-completion-nonessential-deferral.md`, Amend 2026-09-20. `docs/repository-readiness-onboarding.md` documents which `doctor` flag writes which file (`--json` vs `--fix-safe` vs `--refresh-profile`), and `docs/consumer-configuration.md`'s "Repo profile" section corrects its blanket read-only claim for the three operator-confirmable subkeys.
- **`hitl-gates` skill split into one procedure page per command.** The 988-line, 104 KB `procedures.md` that `/run-plan`, `/run-plan-all`, `/continue-plan` and `/start-project` all shared is now four one-hop pages: `run-plan-tick-contract.md`, `start-project-intake.md`, `run-plan-all-queue.md`, and `/continue-plan`'s own 11-line resume procedure folded straight into `.cursor/commands/continue-plan.md` (it already restated most of the same steps). `procedures.md` stays as a short redirect so an existing consumer overlay copy lands somewhere useful on its next `agent-kit update`. `SKILL.md` gains `disable-model-invocation: true` (it listed nearly every command in its trigger description, which made it effectively always-on despite being nominally model-invoked; the sibling `core/qa/SKILL.md` already had this). Reading cost per command drops 48-87% (each command's own page plus `SKILL.md`, instead of the shared 104 KB file every command paid for); total on-disk bytes for the skill plus its four command stubs are flat-to-down versus before the split, not relocated the way #901 was (ADR `decisions/2026-09-20_contract-read-budget-and-laziness.md`).
- **Size pins on every consumer-L0 command, skill page, and generated `.claude` adapter.** New `packages/cli/src/docs/size-budgets.test.ts` pins bytes, lines, and longest line per class (150 lines / 8,192 B / 700 chars for a command; a matching table for skill pages and adapters), proven against a synthetic over-budget fixture so a regression fails CI the way `#901`'s silent regrowth did not. `plan-review-triage.md`, `plan-external-review.md`, `agent-kit-onboard.md`, `field-report-resolve.md`, `backlog-add.md`, and `dashboard-broadcast.md` moved their procedural detail into six new one-hop skill pages to fit; `dashboard.md`, `git-staging.md`, and `git-prod.md` needed a small rewrap. All 25 consumer-L0 commands now comply.
- **`cursor-plan-handoff.mdc` and `context-guardian.mdc` no longer restate the session hard rules and the `/run-plan` tick contract in full.** Both are `alwaysApply` rules loaded on every Cursor session alongside the SessionStart-injected hard rules; an audit found five near-verbatim paragraphs (manual-mode hard stop, `/start-project` gates, backlog CRUD, HANDOFF machine-field format, and the quota-stop recovery mechanics) each said twice. Folded to short pointers, keeping any sentence the other side didn't already say — no change of meaning, one line each rewrapped to fix a longest-line overage as a side effect. Combined `alwaysApply` rule bytes: 38,968 B -> 36,880 B.
- **HANDOFF growth contract: `agent-kit handoff --prune` and a `validate handoff` size warning.** `.cursor/HANDOFF.md` grows without bound because every tick appends a dated entry and nothing removes one. New limit: 200 lines, no line over 2,000 chars (bytes/lines/longest-line, never tokens; replaces `handoff.md`'s old "~500 tokens when possible" phrasing). `agent-kit handoff --prune [--keep N]` (default 5, explicit flag only, never automatic) keeps the N most recent narrative entries and archives the rest to `.cursor/context/archive/` (now gitignored, same class as HANDOFF.md itself), preserving every Mission-Control-parsed machine field byte-identical wherever it sits. `agent-kit validate handoff` warns when either limit is crossed. Proven live on this factory's own HANDOFF (108,272 B / 137 lines -> 52,818 B / 104 lines): a `dashboard-data.mjs` snapshot taken immediately before and after the prune has byte-identical `plans`, `runQueue`, and `checklistNotes`.
- **Docs closeout for the contract read budget work.** `docs/layers-spec.md` gains a "Read budgets and laziness" section naming which files are always-on, load on command, or load on glob, and the numeric budgets each class is pinned to. `docs/capability-inventory.md` records the delta. `docs/consumer-configuration.md`'s `agent-kit run-plan` table cell (was one 6,809-char line) is now a short cell plus a wrapped prose subsection underneath — same content, longest line 1,439 chars. `.cursor/memory/_index.md` (≈237 KiB) and `CHANGELOG.md` (≈286 KiB) are flagged, not fixed, as the same unbounded-growth symptom for a separate future plan.

### Fixed

- **`agent-kit validate plan` accepts the canonical plan template's indentation.** The frontmatter `- id:` check was column-0-only; `.cursor/context/templates/plan.md` indents `todos:` items by two spaces, so a plan written exactly by the template failed with `empty-todos`. The regex is now indent-tolerant, matching `plan-index`'s existing behavior.
