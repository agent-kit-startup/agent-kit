...
- Do not return `kind: resume` (or a resume-shaped skip) as a supported contract. For `/run-plan-all` PO/explore: list eligible-not-queued Gate-A Backlog and adjustment-class hard drift. Omitting them is a gap, not a resume.
